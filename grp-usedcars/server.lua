...
local RSC = GRPCore.RegisterServerCallback
...
RSC('vehsales:TryBuy',function(source,cb,veh)
  ...
  while not xPlayer do xPlayer = GRPCore.GetPlayerFromId(source); Citizen.Wait(0); end
    ...
    if vehData then
      if not forSale[keyData].brought then
        ...
        if xPlayer.identifier ~= veh.owner then
          cb(true,"Você comprou o veículo.")
        else
          cb(true,"Você recuperou o veículo.")
        end
      else
        cb(false,"Alguém está comprando este veículo.")
      end
    else
      cb(false,"Não consigo encontrar este veículo.")
    end
  else
    cb(false,"Não consigo encontrar este veículo.")
  end
...
RSC('vehsales:TrySell', function(source,cb,veh)
  ...
  if not data or not data[1] then
    cb(false,"Você não possui este veículo.")
  else
    if data[1].finance and data[1].finance > 0 then
      cb(false,"Você precisa terminar de pagar este carro antes de poder vendê-lo.")
    else
...
function DoBuy(source,veh)
  ...
  if vData then
    ...
    if vData.owner ~= identifier then
      local xPlayer = GRPCore.GetPlayerFromIdentifier(vData.owner)
      local tick = 0
      while not xPlayer and tick < 1000 do
        tick = tick + 1
        xPlayer = GRPCore.GetPlayerFromIdentifier(vData.owner)
        Citizen.Wait(0)
      end
